...
try:
    ...
    print(f"导入自定义模块失败: {e}")
    print("请确保 PhaseNetModel.py 和 utils.py 文件在正确的路径下。")
    sys.exit(1)
...
class ClientSession:
    def __init__(self, session_id, args, shared_fonts, model, detector):
            ...
            try:
                ...
                if faces:
                    ...
                    if face_roi_bgr.size > 0:
                        ...
                        face_tensor_chw = torch.from_numpy(face_roi_resized).permute(2, 0, 1).float()
                ...
                if len(self.frame_buffer_for_model) == SEQUENCE_LENGTH:
                    ...
                    with torch.no_grad():
                        rppg_model_output, _ = self.model(model_input_bcthw)
                        new_rppg_segment_points = rppg_model_output[0].detach().cpu().numpy()
...
class RPGGApplication:
    def __init__(self, args):
        ...
        # 配置TensorFlow显存动态增长 + 上限
        gpus = tf.config.list_physical_devices('GPU')
        if gpus:
            tf.config.experimental.set_memory_growth(gpus[0], True)  # 动态增长
            tf.config.set_logical_device_configuration(              # 硬性上限
                gpus[0],
                # TODO 修改这里的参数灵活调整显存占用
    ...
    def _load_model(self):
        ...
        try:
            ...
            if missing or unexpected:
                print(f"PhaseNet权重加载提示: missing={len(missing)}, unexpected={len(unexpected)}")
                if missing:
                    print(f"缺失权重示例: {missing[:10]}")
                if unexpected:
                    print(f"多余权重示例: {unexpected[:10]}")
    ...
    def create_session(self):
        ...
        with self.session_lock:
            session = ClientSession(session_id, self.args, self.shared_fonts, self.model, self.detector)
...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default="./checkpoints/phasenet_epoch9.pth", 
                      help="预训练PhaseNet权重的路径。")
...
config = {
    ...
    'model_path': './checkpoints/phasenet_epoch9.pth',
    'font_path': 'msyh.ttf',
    'max_sessions': 10
}
...
def indexjc():
    ...
    if not session_id or not rppg_app.get_session(session_id):
        # 创建新会话
        session_id = rppg_app.create_session()
        resp = make_response(render_template('jc.html'))
        resp.set_cookie('session_id', session_id, max_age=3600)  # 1小时有效期
        return resp
